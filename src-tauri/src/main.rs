...
#[serde(rename_all = "camelCase")] // Because frontend expects camelCase like updatedAt
struct Note {
    id: String,
    title: String,
    content: String,
    ...
    if let Ok(entries) = fs::read_dir(notes_dir) {
        for entry in entries.flatten() {
            ...
            if path.is_file() && path.extension().and_then(|s| s.to_str()) == Some("md") {
                ...
                let updated_at = metadata.modified().unwrap_or(SystemTime::now())
                    .duration_since(UNIX_EPOCH).unwrap().as_millis() as u64;
                let created_at = metadata.created().unwrap_or(SystemTime::now())
                    .duration_since(UNIX_EPOCH).unwrap().as_millis() as u64;

                notes.push(Note { id, title, content, updated_at, created_at });
            }
        }
    }
...
#[tauri::command]
fn save_note(app_handle: AppHandle, id: String, title: String, content: String) -> Result<Note, String> {
    let notes_dir = get_notes_dir(&app_handle)?;
    
    let target_filename = if id.is_empty() {
        let base_name = if title.trim().is_empty() { "Untitled Note".to_string() } else { title.replace(&['<', '>', ':', '"', '/', '\\', '|', '?', '*'][..], "").trim().to_string() };
...
#[tauri::command]
fn rename_note(app_handle: AppHandle, id: String, new_title: String) -> Result<Option<String>, String> {
    let notes_dir = get_notes_dir(&app_handle)?;
    ...
    let base_name = if new_title.trim().is_empty() { "Untitled Note".to_string() } else { new_title.replace(&['<', '>', ':', '"', '/', '\\', '|', '?', '*'][..], "").trim().to_string() };
